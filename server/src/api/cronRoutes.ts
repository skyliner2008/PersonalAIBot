...
router.get('/', (req, res) => {
  try {
    const jobs = dbAll('SELECT * FROM cron_jobs ORDER BY created_at DESC');
    res.json(jobs);
  } catch (error: any) {
...
router.put('/:id', (req, res) => {
  ...
  try {
    const existing = dbGet('SELECT id FROM cron_jobs WHERE id=?', [id]);
...
router.delete('/:id', (req, res) => {
  ...
  try {
    ...
    dbRun('DELETE FROM cron_jobs WHERE id = ?', [id]);
...
router.post('/:id/toggle', (req, res) => {
  const { id } = req.params;
  try {
    const job: any = dbGet('SELECT is_active FROM cron_jobs WHERE id = ?', [id]);
    if (!job) return res.status(404).json({ error: 'Cron job not found' });
    
    const newState = job.is_active ? 0 : 1;
    dbRun('UPDATE cron_jobs SET is_active = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [newState, id]);
