...
export async function handleBrowserStart(io: SocketServer, socket: Socket) {
    try {
...
export async function handleBrowserStop(io: SocketServer) {
    try {
        await closeBrowser();
    } catch (e) {
...
export async function handleFbLogin(io: SocketServer, socket: Socket, data: { email: string; password: string }) {
    try {
...
export async function handleFbCheckLogin(socket: Socket) {
    try {
...
export async function handleChatbotStart(io: SocketServer, socket: Socket) {
    try {
        if (!isRunning()) {
            await launchBrowser();
            io.emit('browser:status', { running: true });
...
export function handleChatbotStop(io: SocketServer) {
    stopChatMonitor(io);
    io.emit('chatbot:status', { active: false });
}

export async function handleCommentbotStart(io: SocketServer, socket: Socket) {
    try {
        if (!isRunning()) {
            await launchBrowser();
            io.emit('browser:status', { running: true });
...
export function handleCommentbotStop(io: SocketServer) {
    stopCommentMonitor(io);
    io.emit('commentbot:status', { active: false });
}

export function handleSchedulerStart(io: SocketServer) {
    try {
        startScheduler(io);
...
export function handleSchedulerStop(io: SocketServer) {
    stopScheduler();
...
function queueVoiceAgentTask(socketId: string, task: () => Promise<any>) {
...
function clearVoiceAgentQueue(socketId: string) {
    voiceAgentQueues.delete(socketId);
    log.info('[VoiceAgent] Queue cleared for interruption', { socketId });
}

function extractLiveToolCommand(args: any): string {
    if (args == null) return '';
...
function normalizeBridgeOutput(text: string): string {
    const normalized = String(text || '').replace(/\r\n/g, '\n').trim();
    if (!normalized) return 'No response from Jarvis Agent.';
    ...
    const cleaned = normalized
        ...
        .filter(line => {
            ...
            if (trimmed.startsWith('**') && trimmed.endsWith('**') && trimmed.length < 60) return false;
            if (trimmed.startsWith('###') && trimmed.length < 60) return false;
            return true;
        })
        .join('\n')
        .trim();
    ...
    if (!cleaned) return 'Task processed successfully.';
    
    const sanitized = sanitizeForLiveApi(cleaned);
    if (sanitized.length <= outputMaxChars) return sanitized;
    
    let cutPoint = outputMaxChars;
    while (cutPoint > 0 && isHighSurrogate(sanitized.charCodeAt(cutPoint - 1))) cutPoint--;
...
function sanitizeForLiveApi(text: string) {
    return text
...
function isProviderRateLimitError(message: string): boolean {
...
function applyVoiceLanguagePolicy(command: string, forceThaiResponse = false) {
    const normalized = String(command || '').trim();
    if (!normalized) return normalized;
    if (!forceThaiResponse) return normalized;
    if (ENGLISH_REPLY_HINT_REGEX.test(normalized)) return normalized;
    return [
        '[VOICE_CALL_MODE]',
...
async function executeVoiceAgentCommand(command: string, socketId: string) {
...
async function executeVoiceAgentCommandWithPolicy(command: string, socketId: string, forceThaiResponse: boolean, customChatId?: string): Promise<string> {
    ...
    try {
        ...
        const normalized = normalizeBridgeOutput(output as string);
...
function buildJarvisLiveSystemInstruction(): string {
    try {
...
function setupSocketEventHandlers(io: SocketServer, socket: Socket): void {
  // Send initial status
  socket.emit('status', {
    browser: isRunning(),
  ...
  socket.on('fb:login', (data: { email: string; password: string }) => handleFbLogin(io, socket, data));
...
function handleVoiceEvents(io: SocketServer, socket: Socket) {
  // ========== Gemini Live Voice ============
  socket.on('voice:start', async (payload: any, ack: any) => {
            try {
                ...
                client.on('audioPart', (base64Data: string) => {
                    socket.emit('voice:audio_recv', { data: base64Data });
                });
                client.on('textPart', (text: string) => {
                    socket.emit('voice:text_recv', { text, source: 'live' });
                });
                client.on('toolCall', (functionCalls: any) => {
                ...
                client.on('quotaError', (reason: any) => {
                ...
                try { ack?.({ ok: false, error: String(err?.message || err || 'voice:start failed') }); } catch { /* ignore */ }
            ...
            if (hasAllMention) {
                ...
                queueVoiceAgentTask(socket.id, async () => {
                    ...
                    try {
                        ...
                        const parts: string[] = [];
                        const iconMap: Record<string, string> = { jarvis: '👑', gemini: '🔷', claude: '🟡', codex: '🟢', kilo: '⚪', openai: '🔵', opencode: '📜' };
        ...
        socket.on('voice:audio_send', (data: any) => {
        ...
        socket.on('voice:stop', (payload: any) => {
...
export function attachSocketAuth(io: SocketServer) {
