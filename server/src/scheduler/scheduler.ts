...
export function startScheduler(io: SocketServer): void {
  ...
  maintenanceJob = cron.schedule('0 3 * * *', () => {
    try {
      ...
      try {
        const result = dbRun(
          `DELETE FROM episodes WHERE timestamp < datetime('now', '-90 days')
      ...
      try {
        dbRun(
          `DELETE FROM messages WHERE timestamp < datetime('now', '-60 days')
 ...
 */
export function refreshCronJobs(io: SocketServer): void {
  // Clear existing
  ...
  const recurring = dbAll(
    `SELECT * FROM scheduled_posts WHERE cron_expression IS NOT NULL AND status != 'posted'`
  ) as any[];
  ...
  for (const post of recurring) {
    ...
    const job = cron.schedule(post.cron_expression, async () => {
      try {
        ...
        dbRun('UPDATE scheduled_posts SET status = ?, scheduled_at = datetime("now") WHERE id = ?',
          ['pending', post.id]);
        await processPendingPosts(io);
      } catch (e) {
