...
function mapMediaUrlPathToLocalFile(value) {
    const normalized = String(value || '').replace(/\\/g, '/').trim();
    const mediaPrefix = '/media/';
    if (!normalized.toLowerCase().startsWith(mediaPrefix))
        return null;
...
function tryResolveFileUrl(value) {
...
function tryResolveRelativePath(value) {
    const normalizedDirect = path.resolve(value);
    if (fs.existsSync(normalizedDirect))
...
function tryResolveUploadPath(value) {
...
function resolveVoiceSourcePath(rawFilePath) {
    ...
    for (const source of candidates) {
        ...
        if (mappedMediaPath)
            return path.resolve(mappedMediaPath);
...
function publishFileForVoiceChat(filePath) {
...
function extractVoiceSocketId(userId) {
    const raw = String(userId || '').trim();
    if (!raw.startsWith('voice_'))
        return null;
...
async function main() {
    ...
    app.get('/api/auth/socket-token', socketTokenLimiter, (_req, res) => {
        ...
        const isNgrokHost = hostHeader.includes('.ngrok-free.') ||
...
+------------------------------------------------------------+
    ...
    async function gracefulShutdown(signal) {
        ...
        try {
            ...
            console.log('[Server] Terminal sessions closed');
        }
        catch { /* ignore */ }
        try {
            stopHealthChecker();
        ...
        try {
            ...
            console.error('[Server] Error stopping swarm:', err);
        }
        chatQueue.clear();
        webhookQueue.clear();
        ...
        try {
            ...
            console.debug('[Server] shutdown browser close:', String(e));
        }
