...
export async function writeFileContent({ file_path, content }) {
    ...
    try {
        ...
        if (typeof global.onFileWrittenByTool === 'function') {
            global.onFileWrittenByTool(resolvedPath);
        }
...
// ==========================================
export const replaceCodeBlockDeclaration = {
...
export async function replaceCodeBlock({ file_path, exact_old_string, new_string }) {
    const check = validateFilePath(file_path);
    if (!check.safe)
        return check.reason;
    try {
