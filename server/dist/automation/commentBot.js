 ...
 */
export async function startCommentMonitor(io) {
    if (isMonitoring)
        return;
    isMonitoring = true;
    consecutiveErrors = 0;
...
export function stopCommentMonitor(io) {
    isMonitoring = false;
    if (pollInterval) {
        clearInterval(pollInterval);
        pollInterval = null;
...
async function checkWatchedPosts(io) {
    ...
    for (const watch of watches) {
        if (!isMonitoring || !isRunning())
            return;
        if (watch.replies_count >= watch.max_replies)
            continue;
        let page = null;
        try {
            page = await newPage();
            ...
            for (const comment of comments) {
                ...
                if (watch.replies_count >= watch.max_replies)
                    break;
                const existing = dbGet('SELECT id FROM replied_comments WHERE fb_comment_id = ?', [comment.id]);
                if (existing)
                    continue;
                const aiResult = await aiChat('comment', buildCommentReplyPrompt(postContent, comment.text, comment.author, watch.reply_style || 'friendly'));
            ...
            `, [watch.id, comment.id, comment.author, comment.text, replyText]);
                        dbRun('UPDATE comment_watches SET replies_count = replies_count + 1 WHERE id = ?', [watch.id]);
...
async function getPostContent(page) {
    try {
        const content = await page.$eval('[data-testid="post_message"], [data-ad-preview="message"]', el => el.textContent);
...
async function getNewComments(page, watchId) {
    const comments = [];
    try {
...
export function isCommentMonitorActive() {
    return isMonitoring;
}
//# sourceMappingURL=commentBot.js.map