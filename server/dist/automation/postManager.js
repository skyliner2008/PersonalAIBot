 ...
 */
function updatePostStatus(io, id, status, options = {}) {
...
export async function processPendingPosts(io) {
    ...
    try {
        ...
        const pendingAi = dbAll(`SELECT id, ai_topic FROM scheduled_posts WHERE status = ? AND ai_topic IS NOT NULL AND scheduled_at <= ?`, [POST_STATUS_PENDING, now]);
        for (const post of pendingAi) {
            ...
            while (attempts < maxAttempts) {
                try {
                    ...
                    if (!content)
                        throw new Error('AI content generation returned empty result');
        ...
        const readyPosts = dbAll(`SELECT id, content, target, target_id FROM scheduled_posts WHERE status = ? AND scheduled_at <= ?`, [POST_STATUS_READY, now]);
        for (const post of readyPosts) {
            ...
            while (attempts < maxAttempts) {
                try {
                    ...
                    if (!success)
                        throw new Error('Facebook API returned failure');
...
export function getScheduledPosts(limit = 50) {
    return dbAll('SELECT id, content, ai_topic, post_type, target, target_id, target_name, scheduled_at, cron_expression, status, error_message FROM scheduled_posts ORDER BY scheduled_at DESC LIMIT ?', [limit]);
}
/**
